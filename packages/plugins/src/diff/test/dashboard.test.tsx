...
import { DiffPanel } from '../dashboard/DiffPanel.js';

describe('DiffPanel', () => {
  it('renders equal, removed, and added character spans without cache metrics', () => {
    const { container } = render(<DiffPanel loading={false} error={null} onRetry={vi.fn()} analysis={{
      id: 'current', timestamp: '2026-08-09T06:00:00.000Z', matched_parent_id: 'parent',
  ...
  it('renders a formatted multiline baseline with line numbers', () => {
    render(<DiffPanel loading={false} error={null} onRetry={vi.fn()} analysis={{
      ...
      diff: [{ type: 'insert', value: '{"content":"first\\nsecond"}' }],
    }} />);
    expect(screen.getByRole('rowheader', { name: 'New line 1' })).toHaveTextContent('1');
