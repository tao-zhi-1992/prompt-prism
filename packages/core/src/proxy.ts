...
function serveRootBrandAsset(request: http.IncomingMessage, response: http.ServerResponse): boolean {
  ...
  const packageDir = path.basename(runtimeDir) === 'dist'
    ...
    : path.basename(path.dirname(runtimeDir)) === 'core'
      ...
      : path.resolve(runtimeDir, '../..');
  const absolute = path.join(packageDir, 'assets', asset.file);
...
export async function createPromptPrismCore(options: PromptPrismOptions = {}, plugins: ServerPluginRuntime): Promise<PromptPrismInstance> {
  ...
  await plugins.init({
    ...
    getTraceResult: (id) => trace.result(id, store.captures, (captureId) => store.readCapture(captureId), (adapterId, body) => getProviderAdapter(adapterId).parseRequest(body), (adapterId, body, contentType) => getProviderAdapter(adapterId).parseResponse(body, contentType)),
...
export async function startPromptPrismCore(options: PromptPrismOptions = {}, plugins: ServerPluginRuntime): Promise<StartedPromptPrism> {
  const instance = await createPromptPrismCore(options, plugins);
