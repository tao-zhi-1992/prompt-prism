...
const jsonStyles = {
  ...
  collapseIcon: 'json-expander json-expander--open',
  expandIcon: 'json-expander json-expander--closed',
...
function Headers({ headers }: { headers: RawHeaders }) {
  ...
  return (
    <dl className="raw-headers">
      {entries.map(([name, value]) => (
        <div key={name}><dt>{name}</dt><dd>{Array.isArray(value) ? value.join('\n') : value ?? ''}</dd></div>
      ))}
    </dl>
  );
}
...
function Body({ body, label }: { body: string; label: string }) {
  ...
  if (json) {
    return <JsonView data={json} style={jsonStyles} shouldExpandNode={expandAllNodes} clickToExpandNode aria-label={`${label} JSON body`} />;
  }
  return <pre className="raw-text">{body}</pre>;
}

function RawSection({ kind, meta, headers, body }: {
...
export function RawPanel({ raw, loading, error, onRetry }: {
  raw: RawCapture | null;
  loading: boolean;
  error: string | null;
  onRetry: () => void;
