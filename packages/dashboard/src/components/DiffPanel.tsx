      ...
      <section className="diff-viewer">
        ...
        <ScrollArea.Root className="diff-scroll">
          <ScrollArea.Viewport className="scroll-viewport">
            <ScrollArea.Content>
              <pre className={`diff-code${hasParent ? '' : ' diff-code--baseline'}`}>
                {analysis.diff.map((part, index) => (
                  <span className={hasParent ? `diff-${part.type}` : 'diff-equal'} key={`${index}-${part.type}`}>{part.value}</span>
                ))}
              </pre>
            </ScrollArea.Content>
          </ScrollArea.Viewport>
